			...
			<div class="sort-by-select">
				<select id="categorySelect">
					...
					<c:forEach items="${list}" var="option">
						<i class="${option.attGroupNo}"></i><option value="${option.optionNo}">${option.optionName}</option>
					</c:forEach> 
				</select>
			</div>
		</div>
...
<c:forEach items="${list}" var="option">
	<div data-name="${option.optionName}" data-no="${option.optionNo}" class="accOption" style="display: none;"></div>
</c:forEach>
...
<script src="${pageContext.request.contextPath}/resources/scripts/admin/revenue.js"></script>